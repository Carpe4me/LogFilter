...
public abstract class BaseLogTable extends JTable implements FocusListener, ActionListener, ILogRenderResolver, AdjustmentListener {
    ...
    protected void init() {
        ...
        setShowGrid(false);
        setSelectionBackground(ThemeConstant.getColorTableSelectionBg());
        setBackground(ThemeConstant.getColorTableBg());
    ...
    public void changeSelection(LogInfo target) {
        ...
        selectTargetLogInfoInLine(target);
    }
    ...
    private void selectTargetLogInfoInLine(LogInfo target) {
        ...
        for (int nIndex = 0; nIndex < rowCount; nIndex++) {
            LogInfo logInfo = ((LogFilterTableModel) getModel()).getRow(nIndex);
            if (logInfo.getLine() >= target.getLine() && lastLine <= target.getLine()) {
    ...
    public boolean showColumn(int nColumn, boolean bShow) {
        // 不显示当前parser不支持的column
        boolean supportedColumn = isSupportedColumn(nColumn);
        if (bShow && !supportedColumn) {
            bShow = false;
        }
    ...
    protected class ColumnHeaderListener extends MouseAdapter {
...
//        if(e.getID() == KeyEvent.KEY_RELEASED)
    ...
    public void SetFilterLogFlow(boolean filterLogFlow) {
